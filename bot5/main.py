...
dp = Dispatcher()
current_storeUUID = None
current_storeToken = None

def getToken(uuid):
    global current_storeToken
    current_storeToken = db.getToken(uuid)
    return current_storeToken[0]
...
async def command_start_handler(message: Message) -> None:
    ...
    await message.answer('Получить данные из базы если есть', reply_markup=kb.getLinkFromBD(message.from_user.id))
...
async def echo_handler(message: Message) -> None:
    ...
    if not current_storeUUID: current_storeUUID = db.getFirstStoreUUID(tgId)  # current_storeUUID = получаем первый магазин
    ...
    try:
        ...
        SET = message.text.upper().find('SET')
        if ghost.isStarted():
        ...
        if SET>-1:
            return await message.answer('Получить данные',reply_markup=kb.getLinkFromBD(tgId))
        if WB>-1:
            ans = wb.getWB()
            ...
            if articulText == '0':
                 wb_analiz.getAnaliz('0', current_storeUUID, getToken(current_storeUUID))
                ...
                ans = wb_analiz.getAnaliz(articulText, current_storeUUID, getToken(current_storeUUID))
                await message.answer(ans)
...
async def process_buttons_press(callback: CallbackQuery):
    ...
    if callback.data.find('hop::')>0: # Если выбрали магазин
        ...
        current_storeUUID = callback.data.split('###')[2]
        ans = wb_analiz.getAnaliz(callback.data, current_storeUUID, getToken(current_storeUUID))
